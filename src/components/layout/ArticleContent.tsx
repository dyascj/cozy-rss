...
import { useArticleSwipeNavigation } from "@/hooks/useArticleSwipeNavigation";
...
export function ArticleContent({ onBack, hideToolbar }: ArticleContentProps = {}) {
  ...
  const { selectedArticleId, mobilePanel } = useUIStore();
  ...
  // Mobile swipe navigation - only enabled when viewing content on mobile
  const isMobileContent = mobilePanel === "content";
  const { handlers: swipeHandlers, swipeState } = useArticleSwipeNavigation({
    enabled: isMobileContent,
    onClose: onBack,
  });
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full bg-background transition-opacity duration-150",
        swipeState.isSwiping && swipeState.progress > 0.3 && "opacity-80"
      )}
      {...swipeHandlers}
    >
